.validate_sq <- function(object, type = NULL) {
  ...
  if (length(get_sq_lengths(object)) != length(object))
    stop("'get_sq_lengths()' method must return a vector of the length equal to the number of sequences", call. = FALSE)
  if (!is.list(object))
