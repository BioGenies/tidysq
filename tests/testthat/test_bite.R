...
sq_ami_cln_na <- list(structure(as.raw(c(0xff, 0xff, 0xff, 0xff, 0xff)), original_length = 8),
                      ...
                      structure(as.raw(c(0xff, 0xff, 0xff, 0xff, 0xff)), original_length = 8))
sq_ami_cln_na_2 <- list(structure(as.raw(c(0xff, 0x7f)), original_length = 3),
...
test_that("biting sq with positive indices - not reaching outside range", {
...
test_that("biting sq with positive indices - reaching outside range", {
  ...
  expect_equivalent(out, sq_ami_cln_na)
  expect_warning(out <- bite(sq_ami_cln, 12:14), 
                 "some sequences are subsetted with index bigger than length - NA introduced")
  expect_equivalent(out, sq_ami_cln_na_2)
})

test_that("biting sq with negative indices - not reaching outside range", {
