...
test_that("read_fasta throws warning when ignore_case = FALSE", {
  expect_warning(read_fasta(fasta_file_mixed_case, "dna_bsc"), 
                 "Detected letters that do not match specified type!")
})

test_that("read_fasta throws warning when strange characters detected", {
  expect_warning(read_fasta(fasta_file_unt, "dna_bsc"), 
                 "Detected letters that do not match specified type!")
})
...
test_that("write_fasta keeps line width", {
  write_fasta(long_sq, name[0], fasta_file_out_4, width = 80)
  expect_true(all(lengths(readLines(read_fasta(fasta_file_out_4, "dna_bsc"))) <= 81))
})
