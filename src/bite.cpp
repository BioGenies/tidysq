...
Rcpp::List CPP_bite(const Rcpp::List& x,
    ...
    if (Rcpp::is_true(Rcpp::all(indices > 0))) {
        ...
        util::ResultWrapper<Sq<RCPP_IT>> ret =
                bite<RCPP_IT>(import_from_R(x, NA_letter), Rcpp::as<std::vector<LenSq>>(cpp_indices));
        if (ret.has_message()) {
        ...
        Rcpp::IntegerVector cpp_indices = -indices - 1;
        Sq<RCPP_IT> ret = skip<RCPP_IT>(import_from_R(x, NA_letter), Rcpp::as<std::vector<LenSq>>(cpp_indices));
